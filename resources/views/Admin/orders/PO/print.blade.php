...
<section id="printableArea" class="invoice" style="width:720px;">
	<div class="row">
		<div class="col-xs-12">
			...
			<div class="row">
				<table class="table table-condensed" style="margin-left:20px;width:98%;">
					<thead>
						<tr>
							<th class="" style="width:{{($order->multiple_shipping != 0)?35:50}}%">Description</th>
							<th class="nowrap" style="width:{{($order->multiple_shipping != 0)?5:10}}%">Qty:</th>
							<th class="nowrap" style="width:{{($order->multiple_shipping != 0)?5:10}}%">Rate:</th>
							<th class="nowrap" style="width:{{($order->multiple_shipping != 0)?5:10}}%">Amount:</th>
							<th class="nowrap" style="width:{{($order->multiple_shipping != 0)?10:10}}%">Due Date:</th>
							<th class="nowrap" style="width:{{($order->multiple_shipping != 0)?10:10}}%">Shipping Via:</th>
							@if($order->multiple_shipping != 0)
					...
					@foreach($po_products as $val)
						...
						<tr>
							...
							<td class="" style="width:{{($order->multiple_shipping != 0)?5:10}}%">
								<div class="col-xs-12 no-padding">
									{{$qty}}
								</div>
							</td>
							...
							@if(isset($val->PoProduct))
								...
								<td class="" style="width:{{($order->multiple_shipping != 0)?5:10}}%">{{config('constants.Shipping_option.'.$shipping_option)}}</td>
								...
								<td class="" style="width:30%">
									...
									{{$val->PoProduct->orderPOAddress->shipping_city.', '.$val->PoProduct->orderPOAddress->shipping_state.' '.$val->PoProduct->orderPOAddress->shipping_zipcode . ' ' . $val->PoProduct->orderPOAddress->shipping_country}}<br>

									</div>
								</td>
								@endif
							@else
								<td class="" colspan="2" style="width:25%"></td>
								@if($order->multiple_shipping != 0)
					...
					@endforeach
						...
						<tr>
							...
							<td class="" style="width:{{($order->multiple_shipping != 0)?5:10}}%"></td>
							<td class="" style="width:{{($order->multiple_shipping != 0)?5:10}}%"></td>
							<td class="" style="width:{{($order->multiple_shipping != 0)?5:10}}%"></td>
							<td class="" style="width:{{($order->multiple_shipping != 0)?10:10}}%"></td>
							<td class="" style="width:{{($order->multiple_shipping != 0)?10:10}}%"></td>
							@if($order->multiple_shipping != 0)
								<td class="" style="width:25%"></td>
							@endif
			...
			<div class="row">
				...
				<div class="col-xs-12" style="width:100%">
					<div class="col-xs-9 text-muted" style="margin-top: 15px;width:45%;">
