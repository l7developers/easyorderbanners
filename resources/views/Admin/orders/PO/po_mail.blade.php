...
<section class="content-header">
	<div class="row">
		<div class="col-xs-6 full_w"><h1>PO Mail</h1></div>
		<div class="col-xs-6 full_w">
			<div class="top_btns">
				<a href="{{url('admin/order/po/'.$id)}}" class="btn btn-success btn-sm" style="float: right;">Back to list</a>
			</div>
		</div>
	</div>
</section>
...
<section class="invoice">
	<div class="row">
		<div class="col-xs-12">
		{{Form::model('po_mail_form',['id'=>'po_mail_form'])}}
			<div class="box box-primary">
				<div class="box-header with-border">
				  <h3 class="box-title">Send PO Detail To Vendor</h3>
				</div>
				<!-- /.box-header -->
				<div class="box-body">
					...
					<div class="form-group">
						<div class="input-group">
							...
							{{Form::text('to',$vendor->email,['class'=>'form-control validate'])}}
						</div>
					</div>
					<div class="form-group">
						<div class="input-group">
					...
					<div class="form-group col-sm-5 col-xs-6 upload_img1 file_upload">
						...
						{{Form::hidden('file_name',$pdf['file_name'])}}
						<div class="file-loading">				{{Form::file('files',['class'=>'uploaded_file'])}}
						</div>
					</div>
				</div>
				<!-- /.box-body -->
				<div class="box-footer">
					<div class="col-lg-12 demotext"></div>
...
$('.uploaded_file').fileinput({
	...
	initialPreview: ['{{URL::to("public/pdf/PO/".$pdf["file_name"])}}'],
	initialPreviewConfig: [
			{type: "pdf", downloadUrl: '{{URL::to("public/pdf/PO/".$pdf["file_name"])}}'}, // disable download
		],
	layoutTemplates: {
...
$(document).on('click','.send',function(){
	...
	if(check == 1){
		...
		$.ajax({
			url:'{{url("admin/order/po/send_po")}}',
			type:'post',
			dataType:'json',
			beforeSend: function () {
			  $.blockUI();
			...
			data:$('form#po_mail_form').serialize(),
			success:function(data){
				if(data.status){
					window.location.href = '{{url("admin/order/po/".$id)}}';
					window.close();
				}else{
