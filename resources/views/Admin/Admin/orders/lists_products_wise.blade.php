						...
						<table class="table table-bordered table-hover table-striped addedfeature order_tabel" border="1" style="width:100%;border-collapse:collapse;">
							<thead>
								<tr>
									...
									<th class="nowrap">
										<a href="{{url('admin/order/lists/po/'.$sort)}}" class="sort_link">PO#</a>
									...
									<th class="nowrap">
										<a href="{{url('admin/order/lists/artwork-status/'.$sort)}}" class="sort_link">ArtWork Status</a>
									...
									<th class="nowrap">
										<a href="{{url('admin/order/lists/vendor-status/'.$sort)}}" class="sort_link">Vendor Status</a>
									...
									<th class="nowrap">
										<a href="{{url('admin/order/lists/due-date/'.$sort)}}" class="sort_link">Due Date</a>
										@if($field == 'due_date')
											{!!$arrow!!}
										@else
											<i class="fa fa-arrows-v"></i>
							...
							@if(count($orders)>=1)
								...
								@foreach($orders as $order)
									<tr>
										...
										<td class="nowrap po_td_{{$order->id}}" scope="row">
										@if($order->order->po_status == 1)
											@php
												if($order->po_id != ''){
													$url = url('/admin/order/po/'.$order->po_id);
													$po_name = $order->po_id;
												}
												else{
													$po_name = 'Create PO';
													$url = url('/admin/order/po/create/'.$order->order_id.'/'.$order->product_id);
												}
											@endphp
										...
										<td class="nowrap">
											...
											<div id='vendor_{{$order->id}}'>
												...
												@if(!empty($order->vendor_name))
													<div class="btn btn-xs btn-primary vendor_btn" data="{{$order->vendor_id}}" order-product-id="{{$order->id}}" order-id="{{$order->order->id}}"
													order-product="{{$order->product_id}}" data-toggle="modal" data-target="#assign_vendor_model" title="Assign Vendor">{{$order->vendor_name}}</div>
												@else
													<div class="btn btn-xs btn-warning vendor_btn" data="0" order-product-id="{{$order->id}}" order-id="{{$order->order->id}}" order-product="{{$order->product_id}}" order-po="{{$order->po_id}}" data-toggle="modal" data-target="#assign_vendor_model" title="Assign Vendor">Assign Vendor</div>
												@endif
											</div>
										</td>
										...
										<?php /*<td class="ship_td_{{$order->item_id}} same_ship_{{$order->order->id}}">
											@if(empty($order->tracking_id))
												<button type="button" id="ship_button_{{$order->item_id}}" class="btn btn-xs bg-purple margin book_shipping" data-type="shipping" order-id="{{$order->order->id}}" product_id="{{$order->product_id}}" product_item_id="{{$order->item_id}}" data-toggle="modal" data-target="#book_shipping" title="Book Shipping">Book Shipping</button><br/>
												<i class="fa fa-spinner fa-spin spinner_{{$order->item_id}} hide" style="font-size:24px"></i>
												<span class="error-msg error_span_{{$order->item_id}}" style="color:red;"></span>
											@else
												<b>Tracking Number :</b> {{$order->tracking_id}}
												<br/>
												<b>Shipping Option :</b> {{config('constants.Shipping_option.'.$order->order->shipping_option)}}
												<br/>
												<a href="{{url('public/uploads/orders/ShippingLabels/'.$order->label_image)}}" target="_blank" class="btn btn-xs bg-navy margin">View Label</a>
												
												<a href="{{url('admin/order/tracking/'.$order->tracking_id)}}" class="btn btn-xs bg-olive margin">Track Order</a>
											@endif									
										</td> 
										...
										<td class="nowrap">	
											...
											@endif
											<a href="{{url('/admin/order/delete/'.$order->order->id.'/'.$order->id)}}" class="btn btn-xs btn-danger" title="Delete order" onclick="return confirm('Are you sure to delete this ?')"><i class="fa fa-times" aria-hidden="true"></i></a>

											@if($order->order->qb_id == "" && $order->order->payment_method !="")
											<a href="#" onClick="window.open('{{url('/admin/quickbook/exporttoqb/'.$order->order->id)}}','qbwindow','height=600,width=800,top=10,left=100')" class="btn btn-xs btn-info" title="{{$order->order->id}}">Export to QB</a>
											@endif
							...
							@else
								<tr>
									<td colspan="15"><center><b>No Data Found here</b></center></td>
								</tr>
							@endif
							</tbody>
						</table>
