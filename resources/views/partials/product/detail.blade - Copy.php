		...
		<div class="row">
			<div class="col-sm-6">
				<div class="prdocut_gallery_slider">
					...
					<div class="leftslider_prodct" id="accordion">
						<div class="panel">
			...
			<div class="col-sm-6">
				<div class="product_page">
					<h4>{{$product->name}}</h4>
					<p class="starting">Starting at <strong>${{$product->price}} </strong></p>
					<div class="rating">
						@if($count > 0)
							<div id="rateYo"></div>
					...
					{{Form::model('product_form',['id'=>'product_form'])}}
							...
							<div class="col-xs-6 col-sm-12 col-md-12 col-lg-5 fullwid">
								<div class="finishingoptions">
									...
									@if($product->show_width_height == 1)
										<div class="form-group main_box">{{Form::label('options[printing][0][width]','Width(ft)',array('class'=>'form-control-label'))}}
											<div class="coustomeselect">{{Form::text('options[printing][0][width]',1,['class'=>'form-control option_fields','id'=>'width','placeholder'=>'Enter Width'])}}
											</div>
										</div>
										<div class="form-group main_box">{{Form::label('options[printing][0][height]','Height(ft)',array('class'=>'form-control-label'))}}
											<div class="coustomeselect">{{Form::text('options[printing][0][height]',1,['class'=>'form-control option_fields','id'=>'height','placeholder'=>'Enter Height'])}}
											</div>
										</div>
									@endif
									...
									@if($option['type'] == 1)
										<div class="form-group main_box">	{{Form::label('options[printing]['.$option['id'].']['.str_replace(' ','_',strtolower($option['name'])).']',$option['name'],array('class'=>'form-control-label'))}}
											<div class="coustomeselect">
												<select name="{{'options[printing]['.$option['id'].']['.str_replace(' ','_',strtolower($option['name'])).']'}}" class="option_fields option_custom" id="option_custom{{$option['id']}}">
												@php
												foreach($option['values'] as $value){
													$price = 0;
													if(array_key_exists('price',$value) and !empty($value['price'])){ $price = $value['price']; }
													echo '<option value=\''.trim(htmlentities($value['value'])).'__'.$price.'\' rel="'.$price.'" data-formate="'.$option['price_formate'].'">'.htmlentities($value['value']).'</option>';
												}												
												@endphp
								...
								<div class="finishingoptions">
									...
									@if($option['type'] == 1)
										<div class="form-group main_box">	{{Form::label('options[finishing]['.$option['id'].']['.str_replace(' ','_',strtolower($option['name'])).']',$option['name'],array('class'=>'form-control-label'))}}
											<div class="coustomeselect">
												<select name="{{'options[finishing]['.$option['id'].']['.str_replace(' ','_',strtolower($option['name'])).']'}}" class="option_fields option_custom">
												@php
												foreach($option['values'] as $value){
													$price = 0;
													if(array_key_exists('price',$value) and !empty($value['price'])){ $price = $value['price']; }
						...
						<div class="row">
								...
								<div class="finishingoptions">
									...
									@if($option['type'] == 1)
										<div class="form-group main_box">	{{Form::label('options[production]['.$option['id'].']['.str_replace(' ','_',strtolower($option['name'])).']',$option['name'],array('class'=>'form-control-label'))}}
											<div class="coustomeselect">
												<select name="{{'options[production]['.$option['id'].']['.str_replace(' ','_',strtolower($option['name'])).']'}}" class="option_fields option_custom">
												@php
												foreach($option['values'] as $value){
													$price = 0;
													if(array_key_exists('price',$value) and !empty($value['price'])){ $price = $value['price']; }
							...
							<div class="col-xs-6 col-sm-12 col-md-12 col-lg-5 fullwid pull-right">
								<div class="totla_amount">
									...
									{{Form::hidden('price','')}}
									{{Form::hidden('sub_total',$product->price,['class'=>'sub_total'])}}
						...
						<p class="samllsize">Production Turnaround Time is Standard 3-5 Business Days.</p>
...
$(document).ready(function() {
	...
	orderObj['quantity']=1;
	orderObj['width']=null;
	orderObj['height']=null;
	orderObj['variant_price']=null;


	$('.option_fields').change(function(){
		calculation();//return false;
	});
	...
	function calculation(){
		...
		$.each(combinations, function (index, value) {
			if(index == comb_str)
			orderObj['variant_price']=value.price;
		});
		
		// custom option value price get //
		var custom = new Array();
		/*$(".option_custom").each(function() {
			if($(this).is( ":text" )){ 
				
			}else{
				/* if($('option:selected', this).attr('data-formate') == 'area'){
					alert(1)
				}
				if($('option:selected', this).attr('data-formate') == 'parimeter'){
					alert(2)
				}
				if($('option:selected', this).attr('data-formate') == 'gross'){
					alert(3)
				}
				if($('option:selected', this).attr('data-formate') == 'item'){
					alert(4)
				} */
				
				//alert($('option:selected', this).attr('rel')+' and '+$('option:selected', this).attr('data-formate'));
				
				/*if(typeof(custom[$('option:selected', this).attr('data-formate')]['price']) == "undefined")
				{
					custom[$('option:selected', this).attr('data-formate')]['price'] = new Array();
				}
				
				custom[$('option:selected', this).attr('data-formate')]['price'].push($('option:selected', this).attr('rel')); 
			} 
		});*/
		...
		$.each(orderObj,function(index,val){
			//alert(index+' and '+val)
		});
...
$('input[name="quantity"],#width,#height').keyup(function(e) {
    if($(this).val() < 1)
		$(this).val('1');
});
