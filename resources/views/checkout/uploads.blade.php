...
<section class="product_cart">
	<div class="container">
		...
		<div class="yourcart">
			...
			<input type="hidden" value="thisisakeyforrmfile123" name="key" />
				...
				<div class="yourcartlist">
					<div class="row">
						...
						<div class="col-sm-12 col-md-12 col-lg-4 no-padding">
							...
							@else
								<div class="col-sm-12 upload_img1 file_upload dropzone no-padding file_uploadDiv_{{$val->id}}_0" data-id="{{$val->id}}" data-key="0" data-url="{{ $orderFileUploads[$val->id]['url'] }}" data-object-key="{{ $orderFileUploads[$val->id]['key'] }}">
...
$('.file').on('fileselect', function(event, numFiles, filename) {
    ...
    var objectKey = $(this).parents('.file_upload').attr('data-object-key') + '.' + fileExt;
    console.log(objectKey);
    var $fileInput = $(this);
    $.ajax({
        type: 'get',
        url: '{{url('/uploads/generate')}}/' + objectKey,
        success: function(response) {
            var $fileUpload = $fileInput.parents('.file_upload');
            var $response = JSON.parse(response);
            $fileUpload.attr('data-url', $response.url);
            $fileUpload.attr('data-object-key', objectKey);
        }
    });
});
...
(function() {
	...
	$('#upload').on('click', function(ev) {
        ...
        $('.file_upload').each(function() {
        	var preSignedUrl = $(this).attr('data-url');
        	var objectKey = $(this).attr('data-object-key');
			var id = $(this).attr('data-id');
            var files = document.getElementById("filesInput_" + id + "_1").files;
            var file = files[0];
            ...
            var request = $.ajax({
                ...
                xhr: function() {
                    ...
                    xhr.upload.addEventListener("progress", function(evt) {
                        console.log('UPLOAD PROGRESS FIRED');
        ...
        $.when.apply(this, results).done(function() {
            ...
            $.ajax({
                type: 'POST',
                url: '{{url('/saveFiles')}}/{{ $id }}',
                data: {_token: '{{csrf_token()}}', files: orderFiles},
                success: function() {
